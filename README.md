...
- **用户级守护模式** (v1.10.100+)：通过 Windows 启动文件夹实现开机自启，无需管理员权限。
- **静默后台运行**：利用 VBS 脚本技术实现无窗口静默运行，不干扰日常工作。
- **环境自动清理** (v1.10.75+)：扩展激活时自动清理旧版本进程、锁文件，避免冲突。
- **独立运行**：Guardian 作为独立 Node.js 进程运行，完全解耦 vscode 依赖。
- **崩溃自动恢复**：进程内部内置看门狗逻辑，确保 24/7 不间断服务，崩溃秒级恢复。
...
- **远程操作扩展** (v1.10.113+)：支持远程执行 `/cmd` 指令、取回文件 `/file_get` 以及原生 `/screenshot` 截图。
