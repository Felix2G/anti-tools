...
- **Windows Service 模式** (v1.10.76+)：作为系统服务运行，支持崩溃自动重启（5秒恢复）
- **环境自动清理** (v1.10.75+)：扩展激活时自动清理旧版本进程、服务、锁文件，避免冲突
- **一键重装服务** (v1.10.77+)：检测到版本不匹配时，自动提示重装，支持自动提权
- **独立运行**：Guardian 作为独立 Node.js 进程运行，完全解耦 vscode 依赖
- **开机自启**：Service 模式下由 Windows Service Manager 管理，稳定可靠
- **远程唤醒**：通过 `/boot` 命令在 Telegram 中启动 IDE，采用 `schtasks /IT` 交互模式确保 GUI 窗口可见
- **CDP 支持**：自动传递 `--remote-debugging-port=9000` 参数，确保 CDP 功能可用
- **EPIPE 防护** (v1.10.75+)：完全隔离标准流，消除管道破裂导致的崩溃风险
...
- **Windows Service 可用**：Guardian 现可作为真正的独立进程运行，Service 模式完全就绪
- **24/7 不间断服务**：系统级崩溃恢复，自动重启功能完整实现
