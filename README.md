...
### 💬 AI Chat 智能对话 (v1.11.0+)
内置 AI 聊天面板，支持多种第三方大模型 API：
- **12 个预设模板**：OpenAI / Claude / Gemini / Deepseek / 通义千问 / 智谱 / Kimi / 硅基流动 / 豆包 / Ollama 等
- **统一模型选择器**：按供应商分组显示所有已配置的模型，一键切换
- **流式输出**：实时 SSE 流式响应，支持中断生成
- **代码上下文**：一键发送当前文件或选中代码，代码块一键复制/插入编辑器
- **历史会话**：自动保存对话历史，支持切换和继续历史对话
- **动态模型获取**：自动从 API 接口拉取最新模型列表，告别手动维护
- **配置导入导出**：一键导出/导入 Provider 配置和设置，API Key 可 AES-256 加密保护
- **Token 用量统计**：工具栏实时显示当前会话的输入/输出/总 Token 消耗
- **安全存储**：API Key 使用 VS Code SecretStorage 加密存储

### ☁️ 云同步与 Token 管理 (v1.11.18+)
跨设备同步配置，告别手动搬运：
- **云盘文件夹同步**：将配置保存到 OneDrive / Google Drive / Dropbox 本地同步目录，云盘自动同步到其他设备
- **GitHub Gist 同步**：将配置保存到 GitHub Private Gist，支持自动创建/更新
- **AES-256 加密**：所有同步数据加密传输，敏感信息安全可靠
- **Token 统一管理**：GitHub / OpenVSX 等服务 Token 使用 SecretStorage 加密存储
- **环境变量导入**：一键从系统环境变量批量导入 Token
...
| `/unlock` | 🔐 解锁敏感操作（设密码后需先解锁才能用 /cmd、/file_get） |
| `/lock` | 🔐 手动锁定终端 |
